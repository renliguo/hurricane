...
{ .demo_name = DEMO_NAME,
  ...
  .splash_screen_timeout_ms = 30000
};
...
static void stop_demo_handler(void *arg)
{
  gos_event_unregister_all(display_event_handler);
  gos_event_unregister_all(cloud_event_handler);
  gos_event_unregister_all(weather_report_event_handler);
...
static void draw_current_weather (weather_data_t *data)
{
  ...
  if(gui_result != 0)  
  { 
    GUI_Clear();   //if JEPG does not draw then simply clear the screen
  }
