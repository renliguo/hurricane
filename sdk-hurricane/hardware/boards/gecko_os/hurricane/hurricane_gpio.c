...
gos_result_t hurricane_gpio_init(void)
{
    ...
    if(GOS_FAILED(result, pca9535_set_dir(&pca9535_i2c_device[0], 0xffff)))
    {
    }
    else if(GOS_FAILED(result, pca9535_set_dir(&pca9535_i2c_device[1], 0xffff)))
    {
    }
    // set all output drivers to low
    else if(GOS_FAILED(result, pca9535_write_output(&pca9535_i2c_device[0], 0x0000)))
    {
    }
    else if(GOS_FAILED(result, pca9535_write_output(&pca9535_i2c_device[1], 0x0000)))
    {
...
gos_result_t hurricane_gpio_write(hurricane_gpio_t gpio, bool state)
{
    ...
    if(GOS_FAILED(result, pca9535_write_output(&pca9535_i2c_device[pca_gpio[gpio].device], pca_out[pca_gpio[gpio].device])))
    {
    }
    else if(GOS_FAILED(result, pca9535_set_dir(&pca9535_i2c_device[pca_gpio[gpio].device], pca_dir[pca_gpio[gpio].device])))
    {
    }
