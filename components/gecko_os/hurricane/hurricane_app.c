...
static void hurricane_launch_demo(void);
...
/*
  if(GOS_FAILED(result, gos_load_app_settings_once("hurricane_settings.ini", SETTINGS_MAGIC_NUMBER)))
  {
    GOS_LOG("Failed to load settings, err:%d", result);
  }
*/
...
// Initialize the GPIO Expanders and turn on key components
  hurricane_gpio_init();
  hurricane_gpio_write(HURRICANE_GPIO_3V3_AUX_EN, true);
  hurricane_gpio_write(HURRICANE_GPIO_LCD_BACKLIGHT_EN, true);
  hurricane_gpio_write(HURRICANE_GPIO_LCD_SUPPLY_EN, true);
  hurricane_gpio_write(HURRICANE_GPIO_I2C_MUX, true);
...
int hurricane_get_jpeg_data(void * p, const uint8_t ** ppData, unsigned num_bytes, U32 offset) 
{
  ...
  gos_file_open(p, GOS_FILE_LOCATION_ANY, false, &pFile);   //ToDO:  handle error
